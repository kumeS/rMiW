...
## Model 作成
model <- create3DModel_2L_v1(inputImageSize = c( NNN, NNN, 24, 1))
summary(model)

## plot the DL model
#model %>% plot_model_modi(width=4, height=1.2)
#py_plot_model <- reticulate::import(module = "tensorflow")$keras$utils$plot_model
#py_plot_model(model, to_file=paste0('Model_', formatC(length(list.files(pattern = "Model_")), width = 3, flag = "0"), '.png'), show_shapes=T, show_layer_names=T)
...
#Fit 図
png(filename = paste0('Rplot_', formatC(length(list.files(pattern = "Model_")), width = 3, flag = "0"), '.png'), width = 480, height = 480); plot(history); dev.off()
...
save_model_hdf5(model, filepath="create3DModel_2L_v2_001.h5")
save_model_weights_hdf5(model, filepath="create3DModel_2L_v2_001w.h5")

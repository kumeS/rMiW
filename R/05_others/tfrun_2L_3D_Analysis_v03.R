...
#setwd("~/Rstudio_Keras_test/rMiW_201227/run_results")
#dir("./R")

source("../R/Model_2L_v3.R", local = T)
...
## Model 作成
model <- create3DModel_2L_v3(inputImageSize = c( NNN, NNN, 24, 1))
summary(model)
...
history <- model %>%
  ...
  list(TrainImgDataset$TrainingGT, TrainImgDataset$TrainingGT, TrainImgDataset$TrainingGT),
  batch_size = BATCH_SIZE,
  epochs = EPOCHS,
  verbose = 1, validation_split = 0,
  callbacks = list(callback_early_stopping(monitor = "loss", min_delta = 0, patience = 1000, mode = "auto")))
...
score01 <- model %>%
  evaluate(TrainImgDataset$Training,
           list(TrainImgDataset$TrainingGT, TrainImgDataset$TrainingGT, TrainImgDataset$TrainingGT), verbose = 1)
print(score01)
...
Data.pred <- list(Training=Y_hat[[1]], TrainingGT=TrainImgDataset$TrainingGT)
ImageView3Dresult(ImgArray=Data.pred,
   ImgSection = 1, XYsize = 256, random=T, SaveAll=T)
Data.pred <- list(Training=Y_hat[[2]], TrainingGT=TrainImgDataset$TrainingGT)
ImageView3Dresult(ImgArray=Data.pred,
                  ImgSection = 1, XYsize = 256, random=T, SaveAll=T)
Data.pred <- list(Training=Y_hat[[3]], TrainingGT=TrainImgDataset$TrainingGT)
ImageView3Dresult(ImgArray=Data.pred,
                  ImgSection = 1, XYsize = 256, random=T, SaveAll=T)

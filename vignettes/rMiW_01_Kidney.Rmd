...
## The clustering with 1x1 tiles

```{r echo=TRUE, eval=FALSE}
Img <- Img00@.Data[,,-4]
str(Img)

Img2DClustering <- function(x=Img, Cluster=3, XY=1024){

display(x, method = "raster")
Img00 <- EBImage::resize(x, w = XY, h = XY, filter="bilinear")
str(Img01)
display(Image(Img01, colormode = "color"), method = "raster")

str(Img01)
a <- matrix(unlist(Img01), nrow = XY*XY)
str(a)

b <- matrix(a[,1], nrow=XY)
str(b)
display(Image(b), method = "raster")

b <- matrix(a[,2], nrow=XY)
str(b)
display(Image(b), method = "raster")
...
y <- kmeans(a, centers=Cluster, iter.max = 1000, 
       nstart = 1,algorithm = "Hartigan-Wong")

d <- matrix(y$cluster, nrow=XY)
str(d)
range(d)
rasterImage()
display(Image(d/max(d)), method = "raster")
...
rasterImage(d, 0.5, 0.5, 5.5, 5.5, interpolate=FALSE)
...
## The clustering with 2x2 tiles


## The clustering with 5x5 tiles
